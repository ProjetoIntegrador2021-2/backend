...
def plug_extension(name: str):
    ...
    with open(
        os.path.join(os.getcwd(), "backend", "ext", f"{name}.py"), "w"
    ) as new_extension:
        ext_template = get_template("ext.pyt")
        new_extension.write(ext_template.render())

    settings["default"]["EXTENSIONS"].append(f"backend.ext.{name}:init_app")
...
def plug_database():
    ...
    # project.ext.database
    with open(
        os.path.join(os.getcwd(), "backend", "ext", "database.py"), "w"
    ) as db_module:
