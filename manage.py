...
def runserver(port):
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'export FLASK_APP=backend.app; export FLASK_ENV=development; flask run --port={port}'
        subprocess.run(cmd, shell=True)
    elif os.name == 'nt':
        cmd = f'$env:FLASK_APP = "backend.app"; $env:FLASK_ENV = "development"; flask run --port={port}'
...
def plug_blueprint(name: str, templates: bool):
    ...
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if f'backend.blueprints.{name}.{name}:init_app' in settings['default']['EXTENSIONS']:
